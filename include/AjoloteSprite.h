...
void drawAjoloteRow(TFT_eSprite& target, int x0, int y0, int row, uint16_t color);

// Dibuja el ajolote completo en tamaño original (96x80) en (x0, y0).
void drawAjoloteFull(TFT_eSprite& target, int x0, int y0, uint16_t color);
...
void drawAjoloteScaled(TFT_eSprite& target, int x0, int y0, uint16_t color, int scale);

// Dibuja el ajolote a la mitad del tamaño (48x40) — versión optimizada
// que toma 1 de cada 2 píxeles en cada eje.
void drawAjoloteHalf(TFT_eSprite& target, int x0, int y0, uint16_t color);
